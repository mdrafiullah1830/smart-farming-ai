...
# Forecast moves within +/-2% of the recent mean are noise, not a trend.
...
async def forecast_price(
    ...
    if np.any(history <= 0):
        return MarketForecastResponse(
            status="error", message="All history values must be positive prices"
        )
    ...
    sequence = (
        ...
        else np.pad(history, (window - len(history),), mode="edge")
    )
    scaled = (sequence - low) / (high - low)

    try:
        outputs = MODEL.session.run(
            None, {MODEL.input_name: scaled.reshape(1, window).astype(np.float32)}
        )
    except Exception as exc:  # noqa: BLE001
        logger.exception("market inference failed")
        ...
        for _ in range(request.days):
            prediction = float(
                MODEL.session.run(
                    None,
                    {
                        MODEL.input_name: np.asarray(rolling[-window:], dtype=np.float32).reshape(
                            1, window
                        )
                    },
                )[0].reshape(-1)[0]
